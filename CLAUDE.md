...
- **[해결 시도] Composition child에 "TO PARENT association" 누락** — 테이블 2개가 활성화된 뒤 CDS 활성화 시 `"Child entity 'ZI_VIBE_CODE' does not have a TO PARENT association with 'ZI_VIBE_CODEGROUP'"` 오류가 발생했다(2026-07-10). `zi_vibe_code.ddls.asddls`의 `association [1..1] to ZI_VIBE_CODEGROUP as _CodeGroup`를 **`association to parent ZI_VIBE_CODEGROUP as _CodeGroup`**로 변경했다 — `association to parent`는 SAP 공식 포매터(`abap-cleaner`)의 DDL 키워드 목록에 일반 `association`과 별개로 등록된 전용 문법이다. 다만 이번 수정은 실사용 예시로 직접 대조하지 못하고 키워드 목록 근거만으로 반영한 것이라 다른 수정 건보다 확신도가 낮다 — 이 수정 후에도 같은 오류가 재발하면 바로 알려줄 것.
